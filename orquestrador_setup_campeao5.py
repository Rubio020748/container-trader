...
API_KEY = "U5vYiZrs74ssZBREX7ET7hpv5LjlgTEI8XVgmLDmgwipeSbWITRm7BNEPwPzHlp0"
API_SECRET = "JBvZxaX2s0oTtnM0yQzs0L1ZNNAkeU3IsHcxWhLPFsuwYirLcNnnPwPq8vA1sYn7"
...
CONFIG_BY_SYMBOL = {
    "AVAXUSDT": {
        "brick_size": 0.0050,          # Tamanho em % do preco (LTP %) (ex: 0.005 = 0.5%)
        "stop_loss_bricks": 2,         # Numero de bricks para stop loss
    },
    "ADAUSDT": {
        "brick_size": 0.0050,          # Tamanho em % do preco (LTP %) (ex: 0.005 = 0.5%)
        "stop_loss_bricks": 2,
...
BOT_FILENAME = "bot_setup_campeao5.py"
...
def start_bot(symbol: str):
    ...
    os.makedirs("bot_logs5", exist_ok=True)
    log_file = f"bot_logs5/log_{symbol}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
